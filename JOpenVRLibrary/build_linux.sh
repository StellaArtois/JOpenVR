...
if [ "$1" == "-m64" ]; then
   rm -rf "$SCRIPTDIR/natives/linux/libJOpenVRLibrary64.so"
   ...
   make
   cp "$SCRIPTDIR/linux64/libJOpenVRLibrary64.so" "$SCRIPTDIR/natives/linux/"
   cd "$CURDIR"
elif [ "$1" == "-m32" ]; then
   rm -rf "$SCRIPTDIR/natives/linux/libJOpenVRLibrary.so"
   ...
   make
   cp "$SCRIPTDIR/linux32/libJOpenVRLibrary.so" "$SCRIPTDIR/natives/linux/"
   cd "$CURDIR"
else
